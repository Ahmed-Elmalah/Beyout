...
const PriceCard = ({
  ...
  return (
    <div
      className={`relative flex flex-col h-full group ${isPopular ? "md:-translate-y-4" : ""}`}
    >
      {/* Popular Badge */}
      {isPopular && (
      ...
      {/* Card Container */}
      <div
        className={`h-full rounded-2xl p-6 flex flex-col transition-all duration-300 relative overflow-hidden ${
          isPopular
      ...
      >
        ...
        <div className="mb-6 border-b border-gray-100 dark:border-white/10 pb-6">
          ...
          <h3
            className={`text-3xl font-bold mt-2 ${isCustom ? "text-slate-900 dark:text-accent-gold" : "text-slate-900 dark:text-white"}`}
          >
            {isCustom
              ? "Custom Package"
              : `${price ? price.toLocaleString() : 0} EGP`}
          </h3>
        </div>
          ...
          {isCustom ? (
            ...
            <div className="flex flex-col h-full">
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-6">
                Need a specific setup? Use our interactive builder to select
                exactly what you need for your home.
              </p>
              <ul className="space-y-3 mb-8">
                {["Full Customization", "Instant Quote", "Expert Review"].map(
                  (item, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-3 text-slate-900 dark:text-white text-sm"
                    >
              ...
              <div className="mt-auto">
                <Button className="w-full justify-center bg-accent-goltext-black hover:bg-yellow-500 shadow-gold">
                  Start Building
                </Button>
              </div>
            </div>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {hardware.map((item, i) => (
                  <span
                    key={i}
                    className="text-[10px] font-bold px-2 py-1 rounded bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/10"
                  >
                    {item}
                  </span>
                ))}
              ...
              {/* Features List */}
              <ul
                className="space-y-3 font-body text-gray-600 dark:text-gray-300 text-right mt-2"
                dir="rtl"
              >
                {features.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="mt-1 shrink-0">
                      <MdCheckCircle
                        className={`text-lg ${isPopular ? "text-primary" : "text-gray-500 dark:text-gray-600"}`}
                      />
                    </span>
